def AcrossCheck(matrix, max_prod):
    for i in range(20):
        for j in range(16):
...
def UpDownCheck(matrix, max_prod):
    for i in range(16):
        for j in range(20):
...
def DiagonalCheck(matrix, max_prod):
    for i in range(16):
        for j in range(16):
...
M = [i.split() for i in grid] #Turns each append into a separate list within a big list (grid).
M = [[int(j) for j in i] for i in M] # converts all the 2 digit irregular numbers into regular by using int on the strings like '08' into 8.

max_product = 0
max_product = AcrossCheck(M, max_product)
max_product = UpDownCheck(M, max_product)
max_product = DiagonalCheck(M, max_product)
